...
public class Level5BlueBirdBehaviour : MonoBehaviour
{
    ...
    void Start()
    {
        _npcData = new NpcData(speed: Random.Range(2.0f,4.0f)); //various depending on level
    ...
    private void Update()
    {
        ...
        {
            Level5Counter.birdcount++; //various
            Debug.Log($"Counter added. current count:{Level5Counter.birdcount}"); //various
        }
        else if (previousInZoom == true && currentlyInZoom == false)
        {
            Level5Counter.birdcount--; //various
            Debug.Log($"Counter substracted. current count:{Level5Counter.birdcount}"); //various
        }
    }
