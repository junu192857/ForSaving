...
public class Level3BlueBirdBehaviour : MonoBehaviour
{
    ...
    void Start()
    {
        _npcData = new NpcData(speed: Random.Range(3.0f,5.0f)); //various depending on level
    ...
    private void Update()
    {
        ...
        {
            Level3Counter.birdcount++; //various
            Debug.Log($"Counter added. current count:{Level3Counter.birdcount}"); //various
        }
        else if (previousInZoom == true && currentlyInZoom == false)
        {
            Level3Counter.birdcount--; //various
            Debug.Log($"Counter substracted. current count:{Level3Counter.birdcount}"); //various
        }
    }
